defmodule CogHipChat.Connector do
  ...
  alias CogChat.Room
  alias CogChat.Message, as: ChatMessage
  alias CogChat.Adapter, as: ChatAdapter
  alias CogHipChat.Users
  alias CogHipChat.Provider
  alias CogHipChat.Rooms
  alias CogHipChat.Util
        ...
        {{:ok, room}, state} ->
          case lookup_user(state, jid: sender) do
            {{:ok, nil}, state} ->
              ...
              state
            {{:ok, user}, state} ->
              GenServer.cast(state.provider, {:chat_message, %ChatMessage{id: UUID.uuid4(:hex),
  ...
  defp rejoin_rooms(state) do
    case ChatAdapter.get_provider_state(@provider_name) do
      nil ->
        {:ok, state}
      {:ok, pstate} ->
  ...
  defp join_room(room_name, state) do
    ...
    case Connection.send(state.xmpp_conn, Stanza.join(muc_name, state.xmpp_name)) do
      :ok ->
        ...
        {:ok, pstate} = ChatAdapter.get_provider_state(@provider_name)
        updated = Map.update(pstate, "rooms", [room_name], &(Enum.uniq([room_name|&1])))
        case ChatAdapter.set_provider_state(@provider_name, updated) do
          {:ok, _} ->
            :ok
          error ->
