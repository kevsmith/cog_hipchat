    ...
    receive do
      {:stanza, %Stanza.IQ{id: ^id}=result} ->
        ...
        {:xmlel, "iq", _, [{:xmlel, "query", _, [{:xmlel, "identity", attrs, _}|_]}|_]} ->
            ...
            room = %Room{id: room_jid, name: room_name, provider: "hipchat", is_dm: false}
  ...
  def fetch_api_room(rooms, name, nil) do
    ...
    case response.status_code do
      ...
      200 ->
        ...
        room = %Room{id: Map.get(result, "xmpp_jid"),
...
defimpl String.Chars, for: CogHipChat.Rooms do

  alias CogHipChat.Rooms

  def to_string(%Rooms{rooms: rooms}) do
    "<CogHipChat.Rooms[rooms: #{length(Map.values(rooms))}]>"
  end
